...
// +----------------------------------------------------------------------
// | Author: yangdayao <yangdayao@gmail.com>
// +----------------------------------------------------------------------
...
class Image {
     ...
     * @param bool $showimg  是否显示原图
      +----------------------------------------------------------
     * @return void
      +----------------------------------------------------------
     */
    static function thumb($image, $thumbname, $type='', $maxWidth=100, $maxHeight=100, $interlace=true, $showimg=false) {
        // 获取原图信息
...
$imgSrc 	= $_GET['src'];
$imgSize  	= $_GET['size'];
$showimg	= false; //是否显示原图
if(!empty($imgSize)){
	if($imgSize=='1'){
		$showimg	= true;
	} else {
		$arr_size 	= explode('x', $imgSize);
		$maxWidth 	= $arr_size[0];
		$maxHeight	= $arr_size[1];
	}
...
$resources = $oImage->thumb($imgSrc, null, '', $maxWidth, $maxHeight, '', $showimg);
$oImage->output($resources);
